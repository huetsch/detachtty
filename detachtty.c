...
main(int argc,char *argv[], char *envp[]) {
	...
	int oldpid=0;
	if(pid_file_path) {
	    FILE *fpid=fopen(pid_file_path,"r");
	    char pidbuf[25];
	    if(!fpid) goto no_pid;
	    if(!fgets(pidbuf,(sizeof pidbuf),fpid)) {
		fclose(fpid);
		goto no_pid;
	    }
	    fclose(fpid);
    ...
    no_pid:
	logprintf(MY_NAME,"Can't create \"%s\": does it already exist from a previous run?\n",socket_path);
	bail(MY_NAME,"bind");
    }
	...
	bail(MY_NAME,"detachtty: exec failed");
    } else {			/* parent */
	...
	sigemptyset(&(act.sa_mask));
	act.sa_flags=0;
	sigaction(SIGPIPE,&act,0); /* don't care about PIPE */
